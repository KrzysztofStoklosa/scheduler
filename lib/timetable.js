...
var TimeTable = Base.extend({
  initialize: function (options) {
    ...
    this.rangeBar.on("changing", function (ev, nrange, changed, rangeObj) {
      ...
      task.timeRange.time_start.setWithoutCallback(work_start_val);
      task.timeRange.time_end.setWithoutCallback(work_end_val);
    });
  ...
  setOnTimeChangeFunc: function (task) {
    ...
    task.timeRange.time_start.onChange(onTimeChange);
    task.timeRange.time_end.onChange(onTimeChange);
  },
  ...
  addTask: function (params) {
    ...
    var rangeVal = [
      this.rangeBar.abnormalise(task.timeRange.time_start.get()),
      this.rangeBar.abnormalise(task.timeRange.time_end.get()),
    ];
  ...
  fromJsonObj: function (jsonObj) {
    for (var t in jsonObj.tasks) {
      ...
      t.time_start = moment(t.work_start).format("HH:mm");
      t.time_end = moment(t.work_end).format("HH:mm");
      this.addTask(t);
    }
  },
});

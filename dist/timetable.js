!(function (definition) {
  return typeof exports === "object"
    ...
    : (window.TimeTable = definition(window.jQuery));
})(function ($) {
            ...
            initialize: function initialize(options) {
              ...
              if (typeof options.label === "function") {
                this.$el.text(
                  options.label.call(
                    this,
                    options.perant.normalise(options.value)
                  )
                );
        ...
        function (_dereq_, module, exports) {
          ...
          var Phantom = Range.extend({
            initialize: function initialize(options) {
              ...
              this.on(
                "mousedown.elessar touchstart.elessar",
            ...
            mousedown: function (ev) {
              if (ev.which === 1) {
                ...
                requestAnimationFrame(function () {
                  newRange.$el
                    .find(".elessar-handle:first-child")
                    .trigger(ev.type);
                });
              }
            },
        ...
        function (_dereq_, module, exports) {
          ...
          var Range = Element.extend(vertical).extend({
            initialize: function initialize(options) {
              ...
              if (!this.readonly()) {
                this.$el
                  .prepend('<div class="elessar-handle">')
                  .append('<div class="elessar-handle">');
                this.on(
                  "mouseenter.elessar touchstart.elessar",
                  $.proxy(this.removePhantom, this)
                );
                this.on(
                  "mousedown.elessar touchstart.elessar",
                  $.proxy(this.mousedown, this)
              ...
              if (typeof this.options.label === "function") {
                this.on("changing", function (ev, range) {
                  self.writeLabel(
                    self.options.label.call(
                      self,
                      range.map($.proxy(self.perant.normalise, self.perant))
                    )
                  );
                });
              } else {
            ...
            writeLabel: function (text) {
              this.$el
                .find(".elessar-barlabel")
                [this.options.htmlLabel ? "html" : "text"](text);
            },
            isVertical: function () {
            ...
            mousedown: function (ev) {
              ...
              if ($(ev.target).is(".elessar-handle:first-child")) {
                $("body")
                  .addClass("elessar-resizing")
                  .toggleClass("elessar-resizing-vertical", this.isVertical());
                $(document).on(
                  "mousemove.elessar touchmove.elessar",
                  this.resizeStart(ev)
                );
                ...
                $("body")
                  .addClass("elessar-dragging")
                  .toggleClass("elessar-dragging-vertical", this.isVertical());
                $(document).on(
                  "mousemove.elessar touchmove.elessar",
                  this.drag(ev)
                );
              ...
              var self = this;
              $(document).one("mouseup.elessar touchend.elessar", function (
                ev
              ) {
                ev.stopPropagation();
                ev.preventDefault();
                if (self.hasChanged && !self.swapping)
                  self.trigger("change", [self.range, self.$el, self]);
                self.swapping = false;
                $(document).off(
                  "mouseup.elessar mousemove.elessar touchend.elessar touchmove.elessar"
                );
                $("body").removeClass(
                  "elessar-resizing elessar-dragging elessar-resizing-vertical elessar-dragging-vertical"
                );
              });
            },
            drag: function (origEv) {
              var self = this,
                ...
                mousePos = getEventProperty(
                  this.ifVertical("clientY", "clientX"),
                  origEv
                );
              ...
              return function (ev) {
                ev.stopPropagation();
                ev.preventDefault();
                var mousePos = getEventProperty(
                  self.ifVertical("clientY", "clientX"),
                  ev
                );
                if (mousePos) {
            ...
            resizeEnd: function (origEv) {
              ...
              return function (ev) {
                ...
                if (mousePos) {
                  ...
                  if (size >= minSize) {
                    self.val(
                      [self.range[0], self.range[0] + size / perantSize],
                      { dontApplyDelta: true }
                    );
                  } else if (size <= 10) {
                    self.swapping = true;
              ...
              return function (ev) {
                ...
                if (mousePos) {
                  ...
                  if (size >= minSize) {
                    ...
                    $(document).trigger(opposite + ".elessar");
                    self.$el
                      .find(".elessar-handle:last-child")
                      .trigger(subsequent + ".elessar");
                  }
                }
              };
        ...
        function (_dereq_, module, exports) {
          ...
          var RangeBar = Element.extend(vertical).extend({
            initialize: function initialize(options) {
              ...
              this.on(
                "mouseleave.elessar touchleave.elessar",
                $.proxy(this.removePhantom, this)
              );
              ...
              if (options.bgMark) {
                ...
                if (options.bgMark.count) {
                  ...
                  for (
                    var i = this.abnormalise(this.options.min);
                    i < this.abnormalise(this.options.max);
                    i += this.abnormalise(options.bgMark.interval)
                  ) {
                    this.$markContainer.append(
                      new Mark({
                        label: options.bgMark.label,
              ...
              if (options.indicator) {
                ...
                indicator.val(
                  this.abnormalise(
                    options.indicator(this, indicator, function () {
                      indicator.val(
                        self.abnormalise(options.indicator(self, indicator))
                      );
                    })
                  )
                );
            ...
            normaliseRaw: function (value) {
              return (
                this.options.min + value * (this.options.max - this.options.min)
              );
            },
            ...
            addRange: function (range, data) {
              var $range = Range({
                ...
                snap: this.options.snap
                  ? this.abnormaliseRaw(this.options.snap + this.options.min)
                  : null,
                label: this.options.label,
                rangeClass: this.options.rangeClass,
                minSize: this.options.minSize
                  ? this.abnormaliseRaw(this.options.minSize + this.options.min)
                  : null,
              ...
              var self = this;
              $range
                .on("changing", function (ev, nrange, changed, rangeObj) {
                  ev.stopPropagation();
            ...
            setVal: function (ranges) {
              if (this.ranges.length > ranges.length) {
                for (
                  var i = ranges.length - 1, l = this.ranges.length - 1;
                  i < l;
                  --l
                ) {
                  this.removeRange(l);
              ...
              ranges.forEach(function (range, i) {
                if (self.ranges[i]) {
                  self.ranges[i].val(
                    range.map($.proxy(self.abnormalise, self))
                  );
                } else {
            ...
            calcGap: function (index) {
              var start = this.ranges[index - 1]
                ? this.ranges[index - 1].val()[1]
                : 0;
            ...
            touchstart: function (ev) {
              ev.preventDefault();
              ev.stopPropagation();
              var w = this.options.minSize
                ? this.abnormaliseRaw(this.options.minSize + this.options.min)
                : 0.05;
              var pageStart = getEventProperty(
                this.ifVertical("pageY", "pageX"),
                ev
              );
              var val =
                (pageStart - this.startProp("offset")) / this.totalSize() -
                w / 2;
            ...
            mousemove: function (ev) {
              ...
              if (
                (direct || phantom) &&
                this.ranges.length < this.options.maxRanges &&
                !$("body").is(".elessar-dragging, .elessar-resizing") &&
                !this.readonly()
              ) {
                if (!this.phantom)
                  this.phantom = Phantom({
                    ...
                    minSize: this.options.minSize
                      ? this.abnormaliseRaw(
                          this.options.minSize + this.options.min
                        )
                      : null,
                    rangeClass: this.options.rangeClass,
                  });
                ...
                if (
                  ...
                  this.calcGap(idx) >= this.options.minSize
                ) {
        ...
        function (_dereq_, module, exports) {
          ...
          function SimpleReactiveVar(val) {
            ...
            this.inputBind = function (
              ...
              $input = $pInput;
              bindEventTypes =
                typeof eventTypes !== "Array" ? eventTypes : [eventTypes];
          ...
          var Task = Base.extend({
            initialize: function (params) {
              ...
              this.info = {
                event_type: new SimpleReactiveVar(params.unpaid_minutes),
                event_id: new SimpleReactiveVar(params.job_name),
                description: new SimpleReactiveVar(params.job_id),
            ...
            toJsonObj: function () {
              return {
                ...
                event_type: this.info.unpaid_minutes.get(),
                event_id: this.info.job_name.get(),
                description: this.info.job_id.get(),
            ...
            fromJsonObj: function (jsonObj) {
              ...
              this.info.description.set(jsonObj.job_id);
              this.info.timecard_id.set(jsonObj.timecard_id);
            },
          });
          Task.createTask = function (params) {
            var timeRange = {
              time_start: moment(params.work_start, "HH:mm"),
              time_end: moment(params.work_end, "HH:mm"),
            };
        ...
        function (_dereq_, module, exports) {
          var TimeRangeValidator = {
            validate: function (newTimeRange, tasks) {
              if (
                newTimeRange.work_start.toString() === "Invalid date" ||
              ...
              if (
                newTimeRange.work_start.toString() ===
                newTimeRange.work_end.toString()
              )
                return new this.ValidationError(
              ...
              for (var i in tasks) {
                ...
                for (var j in trKeys) {
                  var trKey = trKeys[j];
                  if (
                    (timeRange.work_start.get() < newTimeRange[trKey] &&
                ...
                if (
                  timeRange.work_start.get().isSame(newTimeRange.work_start) &&
          ...
          var TimeTable = Base.extend({
            initialize: function (options) {
              ...
              this.rangeBar = RangeBar({
                ...
                max:
                  options.max ||
                  moment().startOf("day").add(1, "day").format("LLLL"),
              ...
              this.$el = $("<div class='time-table'></div>").prepend(
                this.rangeBar.$el
              );
              var self = this;
              ...
              this.rangeBar.on("changing", function (
                ev,
                nrange,
                changed,
                rangeObj
            ...
            addTaskFromRange: function (range) {
              var params = {
                time_start: this.rangeBar.normalise(range.range[0]),
                time_end: this.rangeBar.normalise(range.range[1]),
                event_type: "",
                event_id: "",
                description: "",
                timecard_id: "",
            ...
            bindTaskForm: function (task, $form) {
              ...
              var $deleteButton = $form.find(
                "button[data-tt-model=deleteButton]"
              );
              $deleteButton.click(
                $.proxy(function (ev) {
                  ev.preventDefault();
            ...
            unbindTaskForm: function (task) {
              ...
              var $deleteButton = task.$form.find(
                "button[data-tt-model=deleteButton]"
              );
                  ...
                  return (function (superclass) {
                    ...
                    prototype.initialize = function () {
                      if (superclass.prototype.initialize != null) {
                        return superclass.prototype.initialize.apply(
                          this,
                          arguments
                        );
                      } else {
